...
```html
<template>
  <bfr-table title="远程加载数据" :beforeFetch="beforeFetch" :afterFetch="afterFetch" :pagination="pagination" :fetch-setting="fetchSetting" :api="fetchData" :columns="columns">
...
| summaryFunc     | 计算合计行的方法          | (...arg: any) => any[] | — | — | — |
| showSummary     | 是否显示合计行          | boolean | — | — | — |
| canColDrag     | 是否可拖拽列          | boolean | — | — | TODO |
| canRowDrag     | 是否可拖拽行排序          | boolean | — | — | TODO |
| api     | 接口请求对象          | (...arg: any) => Promise< any > | — | — | — |
| beforeFetch     | 请求之前处理参数          |  (params: object) => object | — | — | — |
| afterFetch     | 定义处理接口返回参数          |  (resArr: object[]) => object[] | — | — | — |
| handleSearchInfoFn     | 查询条件请求之前处理函数          |  Function | — | — | TODO |
| fetchSetting     | 接口请求配置，可以配置请求的字段和响应的字段名，见下方全局配置说明           | [FetchSetting](#fetchsetting) | — | — | — |
| immediate     | 初始化后立即请求接口           | boolean | — | — | — |
| emptyDataIsShowTable     | 在存在搜索表单的时候，如果没有数据是否显示表格           | boolean | — | — | TODO |
| searchInfo     | 额外的请求参数           | object | — | — | TODO |
| useSearchForm     | 使用搜索表单           | boolean | — | — | TODO |
| formConfig     | 表单配置           | FormProps | — | — | TODO |
| columns     | 列属性配置           | [BasicColumn](#basiccolumn)[] | — | — | — |
| showIndexColumn     | 显示序号列           | boolean | — | — | — |
| indexColumnProps     | 序号列配置           | [BasicColumn](#basiccolumn) | — | — | — |
| actionColumn     | 操作列配置           | [BasicColumn](#basiccolumn) | — | — | — |
...
| maxHeight     | 表格最大高度，超过展示滚动条，maxHeight存在时，scroll.y属性无效, maxHeight值为string类型时，必须带有单位，不带有单位会出现滚动条，但无法滚动         | string|number | — | — | TODO |
...
| defaultExpandedRowKeys     | 默认展开的行,设置该属性时，需要同时设置rowKey          | string[] | — | — | — |
| expandedRowKeys     | 展开的行，控制属性         | string[] | — | — | — |
| expandedRowRender     | 额外的展开行         | ``` (record: {record: object; index: number; indent: number; expanded: boolean}) => VNodeChild | JSX.Element ``` | — | — | — |
| expandIcon     | 自定义展开图标         | Function(props):VNode \| VNodeChild \| JSX.Element; | — | — | — |
| expandRowByClick     | 通过点击行来展开子行         | boolean | — | — | — |
| expandIconColumnIndex     | 展开的图标显示在哪一列，如果没有 rowSelection，默认显示在第一列，否则显示在选择框后面         | number | — | — | — |
| footer     | 表格尾部         | Function(currentPageData) =>VNodeChild \| JSX.Element\|v-slot | — | — | — |
...
| rowSelection     | 列表项是否可选择       | [RowSelection](#rowselection) | — | null | — |
...
| size     | 表格大小       | default \| middle \| small \| large  | — | default | — |
...
#### slots
slot名称可通过Column的slot属性进行自定义设置，下面的参数名称为推荐名称
| 参数      | 说明          |   域值  |
|---------- |-------------- |-------------- |
...
| flag | column类型 | INDEX \| DEFAULT \| CHECKBOX \| RADIO \| ACTION |   —	| —	|	—	|
